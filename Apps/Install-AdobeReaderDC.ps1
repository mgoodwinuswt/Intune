...
$Installdir = "c:\programdata\mgoodwinuswt\install_adobe"
...
$destination = "$Installdir\AcroRdrDC1801120035_en_US.exe"
Invoke-WebRequest $source -OutFile $destination

# Start the installation when download is finished
Start-Process -FilePath "$Installdir\AcroRdrDC1801120035_en_US.exe" -ArgumentList "/sAll /rs /rps /msi /norestart /quiet EULA_ACCEPT=YES"
